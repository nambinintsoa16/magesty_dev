Init();
   $('.link').on('click',function(){ 
    loding(); 
       ...
       $.post(base_url+'controlleur/detail_vente_com',{date:date,type:type},function(data){
...
function ca_user(){
  $('.com_use').on('click',function(event){
      event.preventDefault();
      let parent=$(this).parent().parent();
      let date=$('.date_select').text();  
     
     $('.tabe_user tr>td,a').each(function(){ 
        $(this).removeAttr('style');
     }); 
    let parents=$(this).parent().attr('style','background-color:#4caf50;color:#ffff');
    $(this).css('color','#fff');
    let user=parent.children().first().text();
    let type=$(this).attr('id');
    loding();   
     $.post(base_url+'controlleur/detail_vente_coms',{date:date,type:type,user:user},function(data){
         $('.vide_com').empty().append(data);
         Tables();
         stopload();
      });
  });
...
function Init(){
      loding(); 
      let date=$('.date_select').text();
      $.post(base_url+'controlleur/detail_vente_com',{date:date,type:'Previ'},function(data){
             $('.panel').empty().append(data);
