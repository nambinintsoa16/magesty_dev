...
(SELECT SUM(prix.Prix_detail * detailvente.Quantite) FROM detailvente,prix,facture WHERE detailvente.Id_prix = prix.Id and detailvente.Facture = facture.Id AND facture.Date like '".date('Y-m')."%' AND (facture.Status like 'livre' AND facture.Matricule_personnel = personnel.Matricule) ) AS 'CAMOIS',
...
public function relanceTombola($requette =array()){
   return $this->db->where($requette)->group_by('facture')->count_all_results('tombola');
}
