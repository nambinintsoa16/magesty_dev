   ...
   if(link!=""){
    ...
    $.post(base_url+'globale/testLink',{liensurfb:link,type:'link'},function(data){
       if (data.exist=='true') {
             ...
             $('.image').empty().append('<a href="?page=ficheclient&client='+data.Code_client+'" Target="_blank"><img style="width:100%;height:100%;" src="'+data.image+'"></a>');
    ...
    function success_insert(idclient){
        $.get('fonction/image.php',{codeclient:idclient},function(data){
               $('.image').empty().append('<a href="?page=ficheclient&client='+data.idclient+'" Target="_blank"><img style="width:100%;height:100%;" src="http://komone-beta.in-expedition.com/images/client/'+data.idclient+'.jpg"></a>');
               $('.contact').empty().append(data.contact);
...
$.post(base_url+'service_clientel/newfacture', function(data) {
            var fact = data.codefact;
$.post(base_url+'service_clientel/sauvemessage',{message:fact,Id_zone:id_zone,id_con:id_con,Type:'vente',sender:'OPL'},function(){
});
$.post(base_url+'service_clientel/enregistre_commande', { Id_discussion: Id_discussion,contact: contact, fact: fact,Id_zone: Id_zone ,date: date, Debut: Debut, Fin: Fin, ville: ville, quartier: quartier, lieulivre: lieulivre, remarque: remarque, produits: detailcommande, client: client,frailivre:frailivre }, function(datas) {
   
    if(datas.message === true){
            $('.fade').modal('hide');
           
            $.post(base_url+'service_clientel/testDiscution',{idclient:localStorage.getItem('codeclient')},function(data){
                if(data.message===true){
...
$('.Ville').autocomplete({
    source: base_url+"service_clientel/autocomplete_ville/"
});
...
$('.Quartier').autocomplete({
    source:base_url+"service_clientel/autocomplete_quartier",
    select: function (e, ui) {
         $('.fade ').modal('hide'); 
        $.post(base_url+'service_clientel/autocomplete_ville',{Quartier:ui.item.value},function(data){
            $.confirm({
                title: 'choisir ville',
                content:data,
                buttons: {
...
function discrict_chose(quartier,ville){
        $.post(base_url+'service_clientel/autocomplete_discrict',{Quartier:Quartier,Ville:Ville},function(data){
            $.confirm({
