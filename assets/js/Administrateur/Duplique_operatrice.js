$(document).ready(function(){
	...
	$('#valider_la_selection').on('click',function(event){
		event.preventDefault();
		let matricule_temp = $("#operatrice").val();
		let matricule = matricule_temp.split("|");
	    matricule = matricule[0].trim();
		let type ="";
		 loding()
        $('.choise').each(function(element){
        	type = $(this).attr("name");
			if($(this).is(':checked')){
				$.post(base_url+"Administrateur/duplicate_operatrice_method",{matricule,type},function(data){
				});
			}
        });
    ...
    function stopload() {
        $('.jconfirm ').remove();
        alertMessage("succès!", "Operatrice duplique.", "success", "btn btn-success");
