...
const ctx = document.getElementById('circles-1');
 ...
 var textCtx = $("#text").get(0).getContext("2d");
	...
	textCtx.fillText("89%", 150, 150);
 $('#select-produit ,#select-question').on('change',function(event){
 	...
 	$.post(base_url+"Administrateur/get_data_chart",{item_select,question_select_text,question_select_id},function(response){
 		if(response.error !='false'){
 			 char_enquette.data.labels = response.question;
  			 ...
  			 textCtx.fillText(response.number+"%", 150, 150);
