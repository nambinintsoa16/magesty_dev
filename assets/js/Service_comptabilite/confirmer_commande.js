$(document).ready(function(){
 $('.Verser').on('click',function(event){
    event.preventDefault();
    let livreur=$('.livreur').val();
    let remarque=$('.Remarquelivre').val();
    let facture=$('.facture').text();
    loding(); 
    $.post(base_url+'Service_comptabilite/livre_vente',{livreur:livreur,remarque:remarque,facture:facture},function(data){
        if(data.message){
            $('.confirmform').addClass('collapse');
            stopload();
            $.alert('<p class="text-center">Vente Verser  avec succès</p>')
        }
    },'json');
 }); 
 $('.Annule').on('click',function(event){
    ...
    loding();  
   $.post(base_url+'Service_comptabilite/annulationCommandes',{nomlivre:nomlivre,facture:facture,remarque:remarque},function(){
          $('.confirmform').addClass('collapse');
          stopload();
