	...
	$('#all').on('click', function (e) {
		...
		$.post(base_url+"/Produit/tabVue", {status:status}, function(data) {
			...
			var Table = $('.listing').DataTable({
        		...
        		ordering: true,
        		paging: true,
			 	ajax: base_url+"Produit/productListAll?statut="+status,
			 	language: {
		...
		if(categorie != null){
			$.post(base_url+"/Produit/tabVue2", {categorie:categorie}, function(data) {
				...
				var Table = $('.listing2').DataTable({
	        		...
	        		ordering: true,
	        		paging: true,
				 	ajax: base_url+"Produit/productListWithStatut?categorie="+categorie,
				 	language: {
	            		url: "https://cdn.datatables.net/plug-ins/1.10.25/i18n/French.json"
	        		},
				});
			...
			$.post(base_url+"/Produit/tabVue", {status:status}, function(data) {
				...
				var Table = $('.listing').DataTable({
	        		...
	        		ordering: true,
	        		paging: true,
				 	ajax: base_url+"Produit/productList?statut="+status,
				 	language: {
	            		url: "https://cdn.datatables.net/plug-ins/1.10.25/i18n/French.json"
	        		},
				});
