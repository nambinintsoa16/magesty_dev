...
$(document).ready(function () {
	...
	$(document).on("click", ".remove-product", function () {
		...
		if (
			productRow.length > 0 &&
			productContainer.find(".product-row").length > 1
		) {
			productRow.remove();
		}
		checkDuplicate();
	...
	$(".type-facture").on("change", function () {
		...
		$(".codePromo option").each(function () {
			...
			$.post(
				base_url + "operatrice/testPrixPromotion",
				{ codePromo },
				function (data) {
	...
	function afficherStatutDiscussion(afficherStatut, datas) {
		...
		if (afficherStatut) {
			$.post(
				base_url + "operatrice/getStatutDiscussion",
				{
					client: localStorage.getItem("codeclient"),
					idPage: localStorage.getItem("pageUsers"),
				},
				function (data) {
					if (data.type == "termier") {
						...
						$(".conten-message").append(
							'<div class="col-md-12 text-center" style="float: none;"><span style="background:#fff;padding:2px 5px">En&nbsp;Attente&nbsp;</span></div><hr style="border: 1px solid;color: #39C0ED;margin-top: -10px" width:100%>'
						);
	...
	function initialiseNomByCodeClient() {
		$.post(
			base_url + "operatrice/detail_clients",
	...
	function insertNouveauDiscussion(idp) {
		if (idp != "vide") {
			$.post(
				base_url + "operatrice/insertNouveauDiscussion",
				{ codeclient: localStorage.getItem("codeclient"), idPage: idp },
				function (data) {
			...
			$.post(
				...
				function (data) {
					if (data.message == true) {
						...
						$(".conten-message").animate(
							{ scrollTop: $(".conten-message").get(0).scrollHeight },
							1000
						);
						afficherStatut = true;
					...
					$.post(
						base_url + "operatrice/sauveRelanceDiscussion",
						{ client: localStorage.getItem("codeclient"), page: page },
						function () {
							stopload();
							alertMessage(
								"Succé",
	...
	function reloadContentMessage(idclient, page) {
		if (page != "vide") {
			...
			$.post(
				...
				alertMessage(
					"Erreur",
					"Une erreur s'est produite contacter le service informatique.",
					"error",
					"btn btn-danger"
				);
				stopload();
	...
	function messageDeBienvenue(id) {
		if (id != "vide") {
			$.post(
				base_url + "operatrice/get_message_bienvenu",
				{ idPage: id },
				function (data) {
					alertMessage(
						"Bienvenue",
						"Faly miarahaba anao tongasoa eto amin page : " + data.Nom_page,
						"success",
						"btn-success"
					);
				},
	...
	function changerpageclient(image, client) {
		$.post(
			base_url + "operatrice/get_liste_page",
			{},
			function (data) {
				var listePage =
					'<form action="" class="formName"><div class="form-group"><select class="selectPage form-control">';
	...
	function selectPage(page) {
		...
		if (page != "vide") {
			...
			$.confirm({
				...
				content:
					"Voulez vous lancer la discussion avec ce client dans la page " +
					nomPage,
				buttons: {
					formSubmit: {
						...
						action: function () {
							$.post(
								base_url + "operatrice/insertDetailPage",
								{
									idPage: page,
									codeClient: localStorage.getItem("codeclient"),
								},
	...
	function choixPages() {
		if (
			localStorage.hasOwnProperty("isNouveau") &&
			localStorage.getItem("isNouveau")
		) {
			$.post(
				base_url + "operatrice/insertDetailPage",
				{
	...
	var tabs = function loadDeb() {
		...
		$.post(
			base_url + "operatrice/test_statut_client",
			{ page: page, DISC: DISC, codeclient: codeclient },
			function (data) {
	...
	$(".chercher").on("keyup", function () {
		...
		$.post(
			base_url + "operatrice/listeclientss",
			{ client: chain },
			function (data) {
	...
	$(".user").on("click", function () {
		$(".PJ").attr("id", "OPL");
		if (
			$(".clientMessage").attr("class") == "form-control clientMessage w-100"
		) {
		...
		$.post(
			base_url + "operatrice/completeTache",
			{ code: tachs },
			function (data) {
				$.confirm({
					title:
						'<p class="text-center" style="font-size:14px;"><b>MESSAGE DU CLIENT</b></p>',
					content:
						'<form><select  class="form-control action">' +
						data +
						"</select></form>",
					buttons: {
						button: {
						...
						formSubmit: {
							...
							action: function () {
								...
								var tache = this.$content
									.find(".action option:selected")
									.text();
								if (!name) {
	...
	$(".Client").on("click", function () {
		$(".PJ").attr("id", "CLT");
		if (
			$("#reponse_client").attr("class") ==
			"form-control typeahead ui-autocomplete-input"
		) {
	...
	function sendMessage(content, date = null, heure = null) {
		...
		$.confirm({
			title:
				'<p class="text-center" style="font-size:14px;"><b>MESSAGE DU CLIENT</b></p>',
			content:
				'<form><textarea id="input" class="form-control message_contes" rows="10">' +
				content +
				"</textarea></form>",
			...
			if (Delete) {
				$.confirm({
					title:
						'<p class="text-center" style="font-size:14px;"><b>MESSAGE DU CLIENT</b></p>',
					content:
						'<form><p><input type="date" class="form-control date"></p><p><input type="time" class="form-control heure"></p></form>',
	...
	$(".observation").on("click", function (event) {
		...
		$.get(
			base_url +
				"operatrice/getAllObservationsByCodeClient?codeClient=" +
				codeClient,
			(data) => {
	...
	$(".datelivre").on("change", function () {
		...
		if (ladate.getDay() == 0) {
			alertMessage(
				"Erreur",
				"Jour non valide! Pas de livraison le dimanche.",
				"error",
				"btn btn-danger"
			);
			$(this).val(" ");
	...
	$(".validproduit").on("click", function (event) {
		...
		if (test_Link_Image($.trim(tempProduit[0])) == true) {
			linkImage =
				"http://magesty-prod.combo.fun/images/produit/'" +
				$.trim(tempProduit[0]) +
				"'.jpg";
		}
		if (test_Link_Image($.trim(tempProduit[0])) == false) {
			linkImage =
				"http://magesty-prod.combo.fun/images/operatrice/default_image.jpg";
		}
		...
		if (typeof $(".prod").html() == "undefined") {
			if ($(".id_facture_collapse").text() == "") {
				$(".table_commande>tbody:last").append(
					'<tr><th class="prod">' +
						...
						tempPrix[1] +
						'</th><th><img class="img img-thumbnail w-5" style="width:50px;" src="' +
						linkImage +
						'"></th><th><button class="btn btn-danger btn-sm suppr"><i class="flaticon-interface-5"></i></button></th><th class="idPrix collapse">' +
						$.trim(tempPrix[0]) +
						"</th></tr>"
				);
			} else {
				$(".table_commande>tbody:last").append(
					'<tr><th class="prod">' +
						...
						'</th><th><img class="img img-thumbnail w-5" style="width:50px;" src="' +
						linkImage +
						'"></th><th><button class="btn btn-primary btn-sm add_produit"><i class="fa fa-plus"></i></button></th><th class="idPrix collapse">' +
						$.trim(tempPrix[0]) +
						"</th></tr>"
				);
				addPRoduit();
			...
			if ($.inArray(tempProduit[0], table) != -1) {
				alertMessage(
					"Erreur",
					"Ce produit existe déjà dans votre bon de commande. Veuillez modifier la quantité pour ajouter une une nouvelle produit.",
					"error",
					"btn btn-danger"
				);
			} else {
	...
	function autres() {
		$(".btn-test").on("click", function (e) {
			...
			if (
				...
				initable();
				$.post(
					base_url + "operatrice/autre_outil",
					{
						option: option,
						options: options,
						type: type,
	...
	function initable() {
		$.post(
			base_url + "operatrice/tableAutre",
			function (data) {
				if (data.message == true) {
	...
	$(".save").on("click", function () {
		...
		loding();
		$.post(
			base_url + "operatrice/nouveau_codeClient",
			{ type: type },
			function (data) {
				uploadImage(data);
			},
	...
	$(".next").on("click", function (event) {
		event.preventDefault();
		$.post(
			base_url + "operatrice/testIfClientPo",
			{ lienFb: $("#liensurfb").val() },
			"json"
		);
		//var page = $('.page').find('option:selected').val();
		let page = localStorage.getItem("pageUsers");
		$(".Client").attr(
			"src",
			base_url + "images/client/" + localStorage.getItem("codeclient") + ".jpg"
		);
		//$('.code_client_ban').empty().append(localStorage.getItem('codeclient'));
		...
		$.post(
			base_url + "operatrice/testDiscution",
			{ idclient: localStorage.getItem("codeclient"), page: page },
			function (data) {
				if (data.message == "false")
					$.post(
						base_url + "operatrice/new_discussion",
						{ client: localStorage.getItem("codeclient") },
						function (datas) {
						...
						buttons: {
							formSubmit: {
								...
								action: function () {
									...
									//ici
									$.post(
										base_url + "operatrice/sauvemessages",
										{
											tache: tache,
											types: types,
											message: name,
	...
	function saveDetail(codeClient) {
		...
		if ($(".commerciale").val() != "") {
			if ($(".coach").val() == "") {
				...
				$.post(
					base_url + "operatrice/save_detail",
					{
						liensurfb: liensurfb,
						identifient: identifient,
						codeclient: codeClient,
						coach: coach[0],
						commerciale: commerciale[0],
					},
					function (data) {
						if (data) {
	...
	function Init_produit() {
		loding();
		$.post(
			base_url + "operatrice/listeclients",
			{},
			function (data) {
				$(".listeclients").empty().append(data.content);
		...
		$.post(
			base_url + "operatrice/listDataBon",
			{ client: localStorage.getItem("codeclient") },
			function (data) {
				...
				if (data == "<option id='0'></option>") {
					$(".bon-Achat-Alert")
						.empty()
						.append(
							"<div class='alert alert-danger w-75 '>Ce client n'a pas de bon d'achat</div>"
						);
				} else {
					$(".bon-Achat-Alert")
						.empty()
						.append(
							"<div class='alert alert-success w-75 '>Ce client benifichie de bon d'achat.</div>"
						);
				}
			}
		);
		...
		$.post(
			base_url + "operatrice/test_discussion_en_cours",
			{ client: localStorage.getItem("codeclient") },
			function (data) {
				if (data.error === false) {
					//
					...
					$.post(
						base_url + "operatrice/test_statut_client",
						{ page: pagechose, DISC: id_conversation, codeclient: codeclient },
						function (data) {
							if (data.message === false) {
								$(".pageusers").empty().append(page_text);
								$(".entetebadge").css("background-color", data.color);
	...
	function fonctiondel() {
		...
		$(".Qua").on("change", function () {
			if ($(this).val() < 1) {
				alertMessage(
					"Erreur",
					"Le nombre de produit ne doit pas être inférieur a 1.",
					"error",
					"btn btn-danger"
				);
				$(this).val("1");
	...
	$(".save_observation").on("click", function (event) {
		...
		if (page != "vide") {
			...
			$.get(
				base_url +
					"operatrice/getPurchaseInfo?codeClient=" +
					codeClient +
					"&appreciation=" +
					appreciation,
				function (response) {
					const purchaseNumber = response.purchaseNumber;
					const numberOfRefusals = response.numberOfRefusals;
					// Endpoint prediction
					const endpointUrl =
						"https://recovery-day-prediction-app-abbd680938a6.herokuapp.com/predict";
					...
					$.ajax({
						...
						data: JSON.stringify({
							...
							approximate_age: parseInt(approximateAge),
							fb_age: parseInt(fbAge),
							client_localisation: clientLocalisation,
							delivery_area: parseInt(deliveryArea),
							price_wishes: isNaN(priceWishes) ? 0 : parseInt(priceWishes),
							appreciation: parseInt(appreciation),
							constraint_customer: parseInt(customerSentiment),
							news: maxValItem.val == 0 ? "other" : maxValItem.name,
							purchase_number: parseInt(purchaseNumber),
							number_of_refusals: parseInt(numberOfRefusals),
						...
						success: function (res) {
							...
							function addBusinessDays(startDate, daysToAdd) {
								...
								// Si le résultat tombe un week-end, ajuste en conséquence
							...
							const resultDate = addBusinessDays(today, addToDays); 
							...
							$.post(
								base_url + "operatrice/saveObservation",
								{
									codeClient: codeClient,
									...
									date: date.getTime(),
									date_relance: resultDate.getTime()
								},
								() => {
									$(".fade").modal("hide");
									stopload();
									alertMessage(
										...
										"Relancez le client le " +formatDate(resultDate) ,
										"success",
										"btn btn-success"
									);
								}
				...
				alertMessage(
					"Erreur",
					"Une erreur s'est produite lors de l'observation.",
					"danger",
					"btn btn-danger"
				);
			});
	...
	$(".enregistre_commande").on("click", function (event) {
		...
		if (page != "vide") {
			...
			if (typeof $(".prod").text() == undefined) {
				alertMessage(
					"Erreur",
					"Veuillez entre au moins une produit.",
					"error",
					"btn btn-danger"
				);
				...
				alertMessage(
					"Erreur",
					"Tranche d'heure de livraison incorrect,veuiilez entrer tranche d'heure valide.",
					"error",
					"btn btn-danger"
				);
			} else if (Debut == "") {
				...
				alertMessage(
					"Erreur",
					"Veuillez remplir tous le champs 'Ville' avant de valider votre transaction.",
					"error",
					"btn btn-danger"
				);
			} else if (quartier == "") {
				alertMessage(
					"Erreur",
					"Veuillez remplir tous les champs 'Quartier' avant de valider votre transaction.",
					"error",
					"btn btn-danger"
				);
			} else if (lieulivre == "") {
				alertMessage(
					"Erreur",
					"Veuillez remplir tous les champs 'Lieu de livraison'> avant de valider votre transaction.",
					"error",
					"btn btn-danger"
				);
			} else {
					...
					function (data) {
						...
						$.post(
							base_url + "operatrice/enregistre_commande",
							{
								...
								District: District,
								page: page,
								cotactlivre: cotactlivre,
								result_mattr: result_mattr,
								bonus: bonus,
								bon_achat: bon_achat,
							...
							function (datas) {
								...
								if (datas.message === true) {
									$.post(
										...
										function (data) {
											if (data.message === true) {
												...
												alertMessage(
													"Succes",
													"Commande enregistrée avec succes.",
													"success",
													"btn btn-success"
												);
											} else {
									...
									alertMessage(
										"Succes",
										"Une erreure ces produis Veuillez recommencer.",
										"error",
										"btn btn-danger"
									);
								}
		...
		$("#prefetch .typeahead").typeahead(null, {
			...
			templates: {
				suggestion: Handlebars.compile(
					'<div class="row pt-1" style="background:#e6ee9c;font-size:12px;border-bottom:solid 1px #ddd;width:800px;padding:10px 5px;color:#000;cursor:pointer"><div class="col-md-12 text-left">{{CodeDiscussion}}-{{Content}}</div></div>'
				),
			},
		});
	}
	...
	function edit_facture() {
		$("div .modify").on("click", function (e) {
			...
			if ($("#lock").attr("class") == "fa fa-unlock-alt") {
				$.post(
					base_url + "operatrice/dettail_vente_modif",
					{ facture: id },
					function (data) {
						if (data.message == "true") {
	...
	function edit_produit() {
		$(".edit_produit").on("click", function (event) {
			...
			$.post(
				base_url + "operatrice/modifquantite",
				{ idvente: facture, quantite: quantite },
				function (data) {}
			);
		});
		...
		$(".updatedate").on("click", function (event) {
			...
			var date = $(".datelivre").val();
			$.post(
				base_url + "operatrice/changedatefacture",
				{ id: facture, date: date },
				function (data) {}
			);
		});
		...
		$(".delete_produit").on("click", function (event) {
			...
			var facture = parent.find(".idfacture").text();
			$.post(
				base_url + "operatrice/annuleproduit",
				{ idvente: facture },
				function (data) {}
			);
		});
	...
	function addPRoduit() {
		$(".add_produit").on("click", function (event) {
			...
			var facture = $(".id_facture_collapse").text();
			$.post(
				base_url + "operatrice/addProduit",
				{ facture: facture, idPrix: idPrix, quantite: quantite },
				function (data) {}
			);
		});
	}
	...
	$(".saveContact").on("click", function (event) {
		...
		var codeclient = localStorage.getItem("codeclient");
		$.post(
			base_url + "operatrice/rendezvous",
			{
				obs: obs,
				...
				daterdv: daterdv,
				heurervd: heurervd,
				contactRvd: contactRvd,
				produitUsers: produitUsers,
				pageUsers: pageUsers,
			},
			function (data) {
				$(".daterdv").val("");
	...
	$(".check").on("click", function (event) {
		...
		if (typeRelance == null) {
			$.post(
				base_url + "operatrice/check",
				{ code_client: code_client, matricule: matricule, page: page },
				function (data) {
			...
			if (typeRelance == "Relance avec achat") {
				...
				$.post(
					base_url + "operatrice/checkDiscussionRelance",
					{ idRelance },
					function (data) {
	...
	function test_Link_Image(images) {
		var retour = false;
		$.get(
			"http://magesty-prod.combo.fun/images/produit/''" + images + "'.jpg",
			function () {
				retour = true;
			}
		);
