$(document).ready(function(){
  ...
  $('.link').on('click',function(){ 
    loding(); 
       ...
       $.post(base_url+'operatrice/detail_vente_com',{date:date,type:type},function(data){
...
function Init(){
      loding(); 
      let date=$('.date_select').text();
      $.post(base_url+'operatrice/detail_vente_com',{date:date,type:'Previ'},function(data){
             $('.panel').empty().append(data);
             Tables();
             stopload();
      });
...
function Tables (){
        $(".dataTable").dataTable({
          "language": {
              "sUrl": "assets/dataTableFr/french.json"
          }
        });
      }
function loding(){ 
