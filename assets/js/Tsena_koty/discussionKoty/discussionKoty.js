$(document).ready(function() {
    ...
    function Init_client() {
        ...
        $.post(base_url + 'Tsena_koty/Viewdiscussion', { codeclient: codeclient }, function(data) {
    ...
    function initialiseNomByCodeClient() {
        $.post(base_url + 'operatrice/detail_clients', { codeclient: localStorage.getItem("codeclient") }, function(data) {
            if (data.message === true) {
                ...
                $('.Client').css('border-top', '7px ridge' + 'grey');
            } else {}
        }, 'json');
    }
    ...
    var tabs = function loadDeb() {
        ...
        $.post(base_url + 'operatrice/test_statut_client', { page: page, DISC: DISC, codeclient: codeclient }, function(data) {
            if (data.message == false) {
                $('.pageusers').empty().append(page_text);
                // $('.entetebadge').css('background-color', 'grey');
    ...
    $(".validproduit").on('click', function(event) {
        ...
        if (typeof($('.prod').html()) == 'undefined') {
            ...
            $('.total').empty().append(tempPrix[1] + ' KOTY');
        } else {
            $('.prod').each(function() {
    ...
    $(".qt").on('change', function() {
        ...
        $('.total .conttotal').empty().append(total + "Koty");
    ...
    $('.next').on('click', function(event) {
        ...
        //var page = $('.page').find('option:selected').val();
        let page = localStorage.getItem('pageUsers');
        $('.Client').attr('src', "https://nambinintsoatest.combo.fun/images/client/" + localStorage.getItem('codeclient') + ".jpg");
        //$('.code_client_ban').empty().append(localStorage.getItem('codeclient'));
    ...
    function Init_produit() {
        ...
        /*$.get(base_url + "images/client/" + localStorage.getItem('codeclient').trim() + ".jpg").done(function() {
            $('.Client').attr("src", base_url + "images/client/" + localStorage.getItem('codeclient').trim() + ".jpg");
        }).fail(function() {
    ...
    function fonctiondel() {
        ...
        function totaltab() {
            ...
            if (typeof($('.tot').html()) !== 'undefined') {
                ...
                let aff = $('.total').empty().append(sum + ' KOTY');
                return (aff);
            } else {
                $('.total').empty().append('00 KOTY');
            }
        }
    ...
    $('.enregistre_commande').on('click', function(event) {
        ...
        if (page != "vide") {
            ...
            var fraisderetrait = $('.fraisderetrait').val();
            ...
            if (typeof($('.prod').text()) == undefined) {
                ...
                $.post(base_url + 'operatrice/newfacture', function(data) {
                    ...
                    $.post(base_url + 'operatrice/enregistre_commande', { Id_discussion: Id_discussion, contact: contact, fact: fact, Id_zone: Id_zone, date: date, Debut: Debut, Fin: Fin, ville: ville, quartier: quartier, lieu_de_livraison: lieulivre, remarque: remarque, produits: detailcommande, client: client, frailivre: frailivre, fraisderetrait: fraisderetrait, District: District, Localite: Localite, page: page, cotactlivre: cotactlivre, result_mattr: result_mattr }, function(datas) {
                        ...
                        if (datas.message === true) {
                            ...
                            $('.fade').modal('hide');
                            stopload();
                            alertMessage('Succes', 'Une erreure ces produis Veuillez recommencer.', 'success', 'btn btn-success');
    ...
    $('.check').on('click', function(event) {
        ...
        $.post(base_url + 'operatrice/check', { code_client: code_client, matricule: matricule, page: page }, function(data) {}, 'json');
    });
    ...
    Table = $(".tablePromokoty").DataTable({
        processing: true,
